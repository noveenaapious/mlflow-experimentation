...
st.set_page_config(
    page_title="Diabetic Talk Model",
    page_icon="🤖", 
    layout="centered",
...
if st.sidebar.button("Launch server"):
    open_mlflow_ui()
    st.sidebar.success("MLflow Server is Live! http://127.0.0.1:8080")
    mlflow.set_experiment("Experimentation with ML Flow")
    open_browser("http://127.0.0.1:8080")

### Setting Ml flow experiment:
exp=st.sidebar.button("Setting Up a New Experiment")
...
st.sidebar.title("Tuning Hyper parameters")
a = st.sidebar.slider('learning_rate',min_value=0.001, max_value=1.0)
b = st.sidebar.slider('max_depth',min_value=0, max_value=8)
...
class Diabetes_m:
    def __init__(self,raw_data):
        ...
        self.training()
    #def __str__(self):
        #return "Accuracy="+str(self.acc) + ", F1_score"+str(self.f1)
    def __call__(self):
...
if st.button("Train"):
    n=Diabetes_m(raw_data)
    st.success("Final model")
